...
const UsersService = {
  ...
  async getSecByReviewId(db, reviewId) {
    const sub = await db("subs").select("*").where({ reviewId });

    const section = await db("sections").select("*").where({ reviewId });
  ...
  async getUserBySubId(db, id) {
    const sub = await db("subs").select("*").where({ id }).first();
    const user = await UsersService.getUserBySectionId(db, sub.sectionId);
    return user;
  },
  async getUserBySectionId(db, id) {
    const section = await db("sections").select("*").where({ id }).first();
    const user = await UsersService.getUserByInstallmentId(
      db,
  ...
  async getUserByFandomId(db, id) {
    const fandom = await db("fandoms").select("*").where({ id }).first();
  ...
  getUserByUserId(db, id) {
    return db("users")
      ...
      .first()
      .then((user) => {
        if (!!user) return UsersService.serializeUser(user);
        else return user;
      });
  },
