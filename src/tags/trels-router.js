...
trelsRouter
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    if (!tagId || !reviewId)
      return res.status(400).json({ error: "Must provide tagId and reviewId" });
...
trelsRouter
  .route("/:tagId/:reviewId")
  .get((req, res, next) => {
      const db = req.app.get("db")
      const {tagId, reviewId} = req.params
      TrelsService.getTrelById(tagId, reviewId)
      .then(trel => {
          if (!trel) return res.status(400).json({error: 'Tag-review relationship not found'})
          return res.status(200).json(trel)
      }).catch(next)
  })
